...
import { localStorageApi, sessionStorageApi } from "../storage/storage";
...
class Http {
  ...
  private static getHeaders(
    ...
    if (hasAuth) {
      let token = localStorageApi.get(StorageKey.ACCESS_TOKEN);
      if (!token) {
        token = sessionStorageApi.get(StorageKey.ACCESS_TOKEN);
      }
